...
Тобто за допомогою `OnPush`, `cd` механізм не перевірятиме компонент, який не зазнав жодних змін, як і його дочірні компоненти.

---

### OnPush + Observables + async pipe

Коли ми працюємо з Angular, `observables` є нашим інструментом номер один для керування даними та змінами стану. Для підтримки `observables`, Angular надає `async` пайп. `Async pipe` підписується на `observable` і повертає останнє значення. Щоб повідомити Angular, що значення змінилося, він викличе метод `markForCheck`, який надходить із класу `ChangeDetectorRef` **(ChangeDetectorRef компонента)**.
...
Отже, як і раніше, якщо ми використаємо `observables` з `async pipe` в темплейті, це працюватиме так само, як якщо б ми використовували клік івент. Він позначить компонент як брудний, а Angular запустить `cd`.

---

### OnPush + Observables + Who is triggering zone.js?

Якщо наші дані змінюються без нашої взаємодії (click, mouseover тощо), ймовірно, десь під капотом виконується `setTimeout` або `setInterval` або виклик `HTTP`, який тригерить `zone.js`.
...
- Ми використовуємо `setTimeout` (щоб пропустити перше завдання, яке виконується, а також тому, що **Angular запускає виявлення змін принаймні один раз за замовчуванням**), а всередині `setTimeout` ми призначаємо значення для `observable` (тобто змінюємо стан).
...
- Запустіть цей код у своїй програмі та побачите, що у веб-переглядачі буде показано лише `[]`.
- Стан додатку зламано 🧨!

Не супер 😄! Але тут постає ще одне запитання:

- **_Навіщо нам позначати всіх предків брудними?_**

  - Причина цього проста: якщо ми не позначатимемо всіх предків як брудні, ми можемо отримати зламаний стан ще швидше. Як?

  - Давайте знову подивимося на наведений вище приклад, але тепер позначимо лише компонент та його дочірні компоненти як брудні.

  - Отже, ми позначаємо для перевірки лише компонент, де відбувся клік, і його дочірні компоненти. У той момент, коли запуститься `cd` механізм, і він дійде по дереву компонентів  до нашого батьківського компонента, до якого застосовано `OnPush` стратегію, побачить, що він не забруднений і пропустіть його.
...
- Ми можемо зробити це за допомогою методу `detectChanges` у класі `ChangeDetectorRef`.
...
- Коли ми використовуємо `markForCheck`, ми просто повідомляємо Angular, що компонент брудний, і більше нічого не відбувається, тому навіть якщо ми викличемо `markForCheck` 1000 разів, це не буде проблемою.
...
- Але це погана ідея робити це для кожного компонента. Тому що, якщо у нас є список зі 100 елементів і ми плануємо виявлення змін для кожного елемента, у нас буде створено 100 завдань для браузера. І це також не добре для продуктивності.
